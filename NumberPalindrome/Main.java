...
public class Main {
    ...
    public static boolean isPalindrome(int number) {
        ...
        while (number > 0) {
            int newSmallest = number % 10; // will pop smalled value
            strBuilt += newSmallest;
            System.out.println(strBuilt);

            int newLargest = number / 10; // will be largest value - smallest
            number = newLargest;
        }

        System.out.println(strBuilt);
        if (strBuilt.equals(Integer.toString(originalNum))) {
            return true;
        } else {
            return false;
        }
