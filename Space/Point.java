...
public class Point {
    ...
    public double distance() {
        double distanceFormula = Math.sqrt((0 - this.x) * (0 - this.x) + (0 - this.y) * (0 - this.y));
        return distanceFormula;
    }

    public double distance(Point another) {
        double distanceFormula = Math
                .sqrt((another.x - this.x) * (another.x - this.x) + (another.y - this.y) * (another.y - this.y));
        return distanceFormula;
    }

    public double distance(int x, int y) {
        double distanceFormula = Math.sqrt((x - this.x) * (x - this.x) + (y - this.y) * (y - this.y));
        return distanceFormula;
    }
}
...
// * Method named distance without any parameters, it needs to return the
// distance between this Point and Point 0,0 as double.

// * Method named distance with two parameters x, y both of type int, it needs
// to return the distance between this Point and Point x,y as double.
