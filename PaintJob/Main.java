...
public class Main {
    ...
    public static int getBucketCount(double area, double areaPerBucket) {
        if (area <= 0 || areaPerBucket <= 0) {
            ...
            double totalBucketsRequired = Math.ceil(area / areaPerBucket);

            return (int) totalBucketsRequired;
        }
    }
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket, int extraBuckets) {
        if (width <= 0 || height <= 0 || areaPerBucket <= 0 || extraBuckets < 0) {
            ...
            double area = width * height;
            double totalBucketsRequired = Math.ceil(area / areaPerBucket);
            totalBucketsRequired -= extraBuckets;
...
// going to work. Bob might also have some extra buckets at home. He also knows
// the area that he can cover with one bucket of paint.

// 1. Write a method named getBucketCount with 4 parameters. The first parameter
// should be named width of type double. This parameter represents the width of
// the wall.

// The second parameter should be named height of type double. This parameter
// represents the height of the wall.

// The third parameter should be named areaPerBucket. This parameter represents
...
// or if extraBuckets is less than 0, the method needs to return -1 to indicate
// an invalid value.
...
// *getBucketCount(-3.4, 2.1, 1.5, 2); → should return -1 since the width
...
// Write another overloaded method named getBucketCount with 3 parameters namely
...
// *getBucketCount(-3.4, 2.1, 1.5); → should return -1 since the width parameter
// is invalid

// *getBucketCount(3.4, 2.1, 1.5); → should return 5 since the wall area is
// 7.14, and a single bucket can cover an area of 1.5.

// *getBucketCount(7.25, 4.3, 2.35); → should return 14 since the wall area is
// 31.175, and a single bucket can cover an area of 2.35.

// 3. In some cases, Bob does not know the width and height of the wall but he
